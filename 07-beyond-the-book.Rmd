...
```{r, cache=T, message=F,warning=F, fig.width = 7, fig.height = 5}
library(statnet)
library(igraph)
library(ggraph)
...
```{r, message=F, warning=F, cache=F, fig.height = 7, fig.width=7}
# First calculate a distance matrix. We divide by
# 1000 so results are in kilometers.
d_mat <- as.matrix(dist(wankarani[, 5:6])) / 1000
...
```{r, message=F, warning=F, cache=T, fig.height = 5, fig.width=7}
